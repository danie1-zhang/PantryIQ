...
def test_optimizer_rejects_empty_available_pantry(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
...
def test_generate_candidate_respects_item_limit(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
    optimizer = MealOptimizer(
        pantry_foods=optimizer_foods,
...
def test_generated_candidate_respects_serving_limits(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
    optimizer = MealOptimizer(
        pantry_foods=optimizer_foods, constraints=standard_constraints, random_seed=42
    )
    candidate = optimizer.generate_candidate_meal()
    assert candidate is not None
...
def test_generated_candidate_respects_total_serving_limit(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
    optimizer = MealOptimizer(
        pantry_foods=optimizer_foods,
...
def test_meal_with_only_proteins_is_invalid(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
...
def test_meal_with_protein_and_carb_is_valid(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
    optimizer = MealOptimizer(pantry_foods=optimizer_foods, constraints=standard_constraints)
...
def test_condiment_only_meal_is_invalid(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
    optimizer = MealOptimizer(pantry_foods=optimizer_foods, constraints=standard_constraints)
...
def test_find_best_meal_prefers_feasible_candidate(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints, monkeypatch
) -> None:
    optimizer = MealOptimizer(
        pantry_foods=optimizer_foods,
        constraints=standard_constraints,
...
def test_find_best_meal_returns_near_feasible_when_needed(
    optimizer_foods: pd.DataFrame, monkeypatch
) -> None:
...
def test_optimizer_is_reproducible_with_same_seed(
    ...
    first = MealOptimizer(
        pantry_foods=optimizer_foods, constraints=standard_constraints, random_seed=123
    )
    second = MealOptimizer(
        pantry_foods=optimizer_foods, constraints=standard_constraints, random_seed=123
    )
    assert first.generate_candidate_meal() == second.generate_candidate_meal()


def test_find_best_meal_rejects_nonpositive_candidate_count(
    optimizer_foods: pd.DataFrame, standard_constraints: NutritionConstraints
) -> None:
    optimizer = MealOptimizer(
        pantry_foods=optimizer_foods,
