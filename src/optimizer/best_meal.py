...
class MealOptimizer:
    ...
    def find_best_meal(self, number_of_candidates: int = 10_000) -> OptimizerResult:
        ...
        for _ in range(number_of_candidates):
            ...
            if evaluation.is_feasible:
                if self._is_better_evaluation(
                    candidate=evaluation,
                    current_best=best_feasible_evaluation,
                ):
                ...
                if self._is_better_evaluation(
                    candidate=evaluation,
                    current_best=best_infeasible_evaluation,
                ):
    ...
    def _build_random_candidate(self) -> dict[str, float]:
        ...
        selected_indices = self.random.sample(
            population=list(available_foods.index), k=number_of_items
        )
    ...
    def _is_better_evaluation(
        candidate: MealEvaluation,
        current_best: MealEvaluation | None,
    ) -> bool:
    ...
    def _prepare_pantry_foods(
        ...
        if missing_columns:
            raise ValueError(
                f"Pantry food data is missing required columns: {sorted(missing_columns)}"
            )
        ...
        foods = foods[
            foods["is_available"] & (foods["servings"] > 0) & (foods["max_servings"] > 0)
        ].copy()
