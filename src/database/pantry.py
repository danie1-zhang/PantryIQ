...
class Pantry:
    ...
    def __init__(self, food_catalog: pd.DataFrame, pantry_items: pd.DataFrame | None = None, pantry_path: str | Path | None = None) -> None:
        """
    ...
    def empty(cls, food_catalog_path: str | Path | None = None, pantry_path: str | Path | None = None) -> Pantry:
        ...
        return cls(food_catalog=food_catalog,pantry_items=None, pantry_path=pantry_path,)
    ...
    def from_csv(cls, pantry_path: str | Path, food_catalog_path: str | Path | None = None) -> Pantry:
        ...
        food_catalog = cls._read_csv(catalog_path, "food catalog")
        return cls(food_catalog=food_catalog, pantry_items=pantry_items, pantry_path=pantry_path,)
    ...
    def _prepare_food_catalog(self, food_catalog: pd.DataFrame) -> pd.DataFrame:
        ...
        if catalog["food_item_id"].duplicated().any():
            duplicate_ids = catalog.loc[catalog["food_item_id"].duplicated(), "food_item_id",].tolist()
    ...
    def _prepare_pantry_items(self, pantry_items: pd.DataFrame | None) -> pd.DataFrame:
        ...
        unknown_ids = set(pantry["food_item_id"].astype(str)) - set(self.food_catalog["food_item_id"].astype(str))

        if unknown_ids:
            raise ValueError("Pantry contains food IDs not found in the catalog: "f"{sorted(unknown_ids)}")

        pantry["is_available"] = (pantry["is_available"].astype(str).str.strip().str.lower().map(
            {
                    "true": True,
                    "false": False,
                    "1": True,
    ...
    def add_food(self, food_query: str, servings: float, max_servings: float | None = None, notes: str = "", date_added: date | None = None,) -> None:
        """
        ...
        if existing_mask.any():
            ...
            if notes.strip():
                ...
                self.pantry_items.at[row_index, "notes"] = (f"{old_notes}; {notes.strip()}" if old_notes else notes.strip())
        else:
            new_row = {
    ...
    def input_foods(self) -> None:
        ...
        while True:
            ...
            max_servings = self._prompt_positive_number("What is the maximum number of servings allowed in one meal? ")
            ...
            try:
                self.add_food(food_query=food_query, servings=servings, max_servings=max_servings, notes=notes,)
            except ValueError as exc:
    ...
    def _find_catalog_food(self, food_query: str) -> pd.Series:
        ...
        name_matches = self.food_catalog[self.food_catalog["food_nm"].astype(str).str.casefold() == query.casefold()]
        ...
        if len(name_matches) > 1:
            ...
            raise ValueError(f"Multiple foods are named '{query}'. Use one of these IDs: {matching_ids}")
    ...
    def _refresh_nutrition_facts(self) -> None:
        ...
        catalog_without_name = self.food_catalog.drop(columns=["food_nm"], errors="ignore",)
        joined = self.pantry_items.merge(catalog_without_name, on="food_item_id", how="left", validate="many_to_one",)
    ...
    def available_items(self) -> list[str]:
        ...
        if self.pantry_items.empty:
            return []
        available = self.pantry_items[self.pantry_items["is_available"] & (self.pantry_items["servings"] > 0)]
    ...
    def available_items_df(self) -> pd.DataFrame:
        ...
        return self.items_nutrition_facts[self.items_nutrition_facts["is_available"] & (self.items_nutrition_facts["servings"] > 0)].reset_index(drop=True)
    ...
    def to_csv(self, path: str | Path | None = None) -> Path:
        ...
        if output_path is None:
            raise ValueError("No pantry CSV path was provided. Pass a path to to_csv() or initialize the Pantry with pantry_path.")

        output_path.parent.mkdir(parents=True, exist_ok=True)
        self.pantry_items.to_csv(output_path, index=False, date_format="%Y-%m-%d",)
